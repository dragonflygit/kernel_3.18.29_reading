...
#define DOMAIN_LEN	(64+1)
#define COOKIE_LEN	(64+1)
#define PATH_LEN	(128+1)
#define CONTENT_TYPE_LEN	(48+1)
...
typedef struct qn
{
	uint32_t src_ip;
	uint32_t dst_ip;
	uint32_t req_type;	//0-not sure, 1-GET, 2-POST
	char* con_type;		//content type
	char* path;			//url path
	char* cookie;		//cookie
	char* value;		//domain,host
	struct timeval time;
	struct qn* next;
	struct qn* previous;	
} queue_node;
...
void add_queue_node(uint32_t src_ip, char* value, queue* full_queue, string_map* queue_index, char* queue_index_key, uint32_t max_queue_length,
		uint32_t req_type, char* con_type, char* cookie, char* path, uint32_t dst_ip)
{

	queue_node *new_node = (queue_node*)kmalloc(sizeof(queue_node), GFP_ATOMIC);
	char* dyn_value = kernel_strdup(value);
	struct timeval t;
	char* dyn_con_type = kernel_strdup(con_type);
	char* dyn_cookie = kernel_strdup(cookie);
	char *dyn_path = kernel_strdup(path);

	if(new_node == NULL || dyn_value == NULL || dyn_con_type == NULL || dyn_cookie == NULL || dyn_path == NULL) 
	{
		if(dyn_con_type) { kfree(dyn_con_type); }
		if(dyn_cookie) { kfree(dyn_cookie); }
		if(dyn_path) { kfree(dyn_path); }
	...
	new_node->req_type = req_type;
	new_node->con_type = dyn_con_type;
	new_node->cookie = dyn_cookie;
	new_node->path = dyn_path;
	...
	if( full_queue->length > max_queue_length )
	{
		...
		kfree(old_node->con_type);
		kfree(old_node->cookie);
		kfree(old_node->path);
		kfree(old_node);
	}
...
{	
	...
	while(last_node != NULL)
	{
		...
		free(last_node->con_type);
		free(last_node->cookie);
		free(last_node->path);
		free(last_node);
...
static void extract_url(const unsigned char* packet_data, int packet_length, char* domain, char* path, char* cookie, char* con_type)
{
	...
	if(path_end_index > 0) 
	{
		...
		path_length = path_length < PATH_LEN ? path_length : (PATH_LEN - 1); /* prevent overflow */
	...
	if(domain_match != NULL)
	{
		...
		domain_end_index = domain_end_index < DOMAIN_LEN ? domain_end_index : (DOMAIN_LEN - 1); /* prevent overflow */
	...
	/* get cookie */
	cookie_match = strnistr( (char*)packet_data, "Cookie:", last_header_index);
	if(cookie_match != NULL)
	{
		int cookie_end_index;
		cookie_match = cookie_match + 7; /* character after "Host:" */
		while(cookie_match[0] == ' ' && ( (char*)cookie_match - (char*)packet_data) < last_header_index)
		{
			cookie_match = cookie_match+1;
		}
		
		cookie_end_index = 0;
		while(	cookie_match[cookie_end_index] != '\n' && 
			cookie_match[cookie_end_index] != '\r' && 
			((char*)cookie_match - (char*)packet_data)+cookie_end_index < last_header_index 
			)
		{
			cookie_end_index++;
		}
		cookie_end_index = cookie_end_index < COOKIE_LEN ? cookie_end_index : (COOKIE_LEN - 1); /* prevent overflow */
		memcpy(cookie, cookie_match, cookie_end_index);
		cookie[cookie_end_index] = '\0';

		for(cookie_end_index=0; cookie[cookie_end_index] != '\0'; cookie_end_index++)
		{
			cookie[cookie_end_index] = (char)tolower(cookie[cookie_end_index]);
		}
	}
	/* get content_type */
	con_type_match = strnistr( (char*)packet_data, "Content-Type:", last_header_index);
	if(con_type_match != NULL)
	{
		int con_type_end_index;
		con_type_match = con_type_match + 13; /* character after "Host:" */
		while(con_type_match[0] == ' ' && ( (char*)con_type_match - (char*)packet_data) < last_header_index)
		{
			con_type_match = con_type_match+1;
		}
		
		con_type_end_index = 0;
		while(	con_type_match[con_type_end_index] != '\n' && 
			con_type_match[con_type_end_index] != '\r' && 
			((char*)con_type_match - (char*)packet_data)+con_type_end_index < last_header_index 
			)
		{
			con_type_end_index++;
		}
		con_type_end_index = con_type_end_index < CONTENT_TYPE_LEN ? con_type_end_index : (CONTENT_TYPE_LEN - 1); /* prevent overflow */
		memcpy(con_type, con_type_match, con_type_end_index);
		con_type[con_type_end_index] = '\0';

		for(con_type_end_index=0; con_type[con_type_end_index] != '\0'; con_type_end_index++)
		{
			con_type[con_type_end_index] = (char)tolower(con_type[con_type_end_index]);
		}
	}
}
...
static int webmon_proc_domain_show(struct seq_file *s, void *v)
{
	...
	{
		seq_printf(s, "%ld\t"STRIP"\t"STRIP"\t\"%s\"\t%u\t\"%s\"\t\"%s\"\t\"%s\"\n", 
				(unsigned long)(next_node->time).tv_sec, NIPQUAD(next_node->src_ip), NIPQUAD(next_node->dst_ip), 
				next_node->value, next_node->req_type,next_node->con_type,next_node->path,next_node->cookie);
...
static int webmon_proc_search_show(struct seq_file *s, void *v)
{
	...
	while(next_node != NULL)
	{
		seq_printf(s, "%ld\t"STRIP"\t"STRIP"\t\"%s\"\t\"%s\"\t\"%s\"\n", 
				(unsigned long)(next_node->time).tv_sec, NIPQUAD(next_node->src_ip), NIPQUAD(next_node->dst_ip), 
				next_node->value, next_node->path, next_node->cookie);
		next_node = (queue_node*)next_node->previous;
	}
				...
				for(line_index=0; line_index < num_lines; line_index++)
				{
					...
					if(length == 3)
					{
						...
						if(sscanf(split[0], "%ld", &time) > 0 && valid_ip == 4)
						{
							...
							if(type == WEBMON_DOMAIN)
							{
								add_queue_node(ip, value, recent_domains, domain_map, value_key, max_domain_queue_length, 0, "0", "0", "0", 0);
								(recent_domains->first->time).tv_sec = time;
							}
							else if(type == WEBMON_SEARCH)
							{
								add_queue_node(ip, value, recent_searches, search_map, value_key, max_search_queue_length, 0, "0", "0", "0", 0);
...
static bool match(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if(iph->protocol == IPPROTO_TCP)
	{
		...
		{
			if (strnicmp((char*)payload, "GET ", 4) == 0)
				req_type = 1;

			if (strnicmp((char*)payload, "POST ", 5) == 0)
				req_type = 2;

			/* are we dealing with a web page request */
			if(req_type != 0 || strnicmp((char*)payload, "HEAD ", 5) == 0)
			{
				char con_type[CONTENT_TYPE_LEN + 1] = "";
				char cookie[COOKIE_LEN + 1] = "";
				char domain[DOMAIN_LEN + 1] = "";
				char path[PATH_LEN + 1] = "";
				char domain_key[DOMAIN_LEN + PATH_LEN + 32 + 1];	
				...
				if(save)
				{
					extract_url(payload, payload_length, domain, path, cookie, con_type);
					...
					if(strlen(domain) > 0)
					{
						...
						{
							//add
							add_queue_node(iph->saddr, domain, recent_domains, domain_map, domain_key, max_domain_queue_length, req_type, con_type, cookie, path, iph->daddr);
						}
						
							
						/* printk("domain,path=\"%s\", \"%s\"\n", domain, path); */

						if(strnistr(domain, "google.", DOMAIN_LEN) != NULL)
						{
						...
						{
							int spi, si;
							char search_key[PATH_LEN + 32 + 1];
							char search[PATH_LEN + 1];
							queue_node *recent_node = recent_searches->first;
							
							/*unescape, replacing whitespace with + */
							si = 0;
							for(spi=0; search_part[spi] != '\0' && search_part[spi] != '&' && search_part[spi] != '/' && spi < PATH_LEN; spi++)
							{
								int parsed_hex = 0;
								if( search_part[spi] == '%')
								{
							...
							{
								//add
								add_queue_node(iph->saddr, search, recent_searches, search_map, search_key, max_search_queue_length, req_type, "0", domain, path, iph->daddr);
							}
						}
						spin_unlock_bh(&webmon_lock);
					}
